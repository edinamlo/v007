...
def convert_tuples_to_lists(obj):
    """Convert any tuples in the object to lists for JSON serialization."""
    if isinstance(obj, tuple):
        return list(obj)
    elif isinstance(obj, dict):
        return {str(k): convert_tuples_to_lists(v) for k, v in obj.items()}
...
def main():
    ...
    result = convert_tuples_to_lists(result)
