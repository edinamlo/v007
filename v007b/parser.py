...
# NEW: support formats like 1x01, 8x12, etc.
EPISODE_ALT_RE = re.compile(r"(?i)(?<![A-Za-z0-9])(\d{1,2}x\d{1,3})(?![A-Za-z0-9])")

_RIGHT_SEP_TRIM = re.compile(r"[.\-\s_\(\)\[\]]+$")

# NEW: strip common website/release prefixes before tokenizing
...
def _collect_matches(token: str) -> List[Tuple[int, int, str, str]]:
    ...
    PATTERNS = [
        (EPISODE_RE, "episode"),
        (EPISODE_ALT_RE, "episode"),   # added alt episode pattern
        (TV_CLUE_RE, "tvclue"),
        (SEASON_RE, "tvseason"),
...
def parse_filename_internal(filename: str, quiet: bool = False) -> dict:
    ...
    # NEW: strip common prefixes like [site] or www.site - before tokenizing
    name = _PREFIX_RE.sub("", name)

    # CHANGED: split on common non-alphanumeric separators so dot/underscore tokens become separate
    ...
    # preliminary pass: if any token contains episode/tv clue mark as TV to prefer over year
    for tok in tokens:
        for regex in (EPISODE_RE, EPISODE_ALT_RE, TV_CLUE_RE, SEASON_RE, CHAPTER_RE):
            if regex.search(tok):
                is_tv = True
                break
        if is_tv:
    ...
    while i >= 0:
        ...
        # If TV already detected, ignore movieyear matches in this token
        if is_tv and matches:
            matches = [mm for mm in matches if mm[2] != "movieyear"]

        # If movie already found, ignore further movieyear matches (existing behavior)
        ...
        for start, end, typ, text in matches:
            ...
            if typ == "episode":
                ...
                if not movie_found and not is_tv:
