...
def cleaner_1_square_brackets_and_media_type_clues_for_anime(
    ...
    4. If it's not a website, check if it contains a known anime release group
    5. Mark as anime if ANY part of the filename contains a known anime release group
    6. REMOVE the processed bracket from the output filename
    ...
    # Now check the ENTIRE filename for anime release groups
    anime_group_found = None
    for group in KNOWN_CLUES["release_groups_anime"]:
        if re.search(re.escape(group), filename, re.IGNORECASE):
