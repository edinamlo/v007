...
def cleaner_2_replace_square_brackets_at_end(
    ...
    Preserves resolution information by not removing it from brackets.
    """
    original = filename
    metadata = {
        "found": [],
    ...
    if bracket_items:
        ...
        for item in bracket_items:
            ...
            if is_anime_group:
                ...
                # Check if it's a resolution pattern
                is_resolution = False
                ...
                if is_resolution:
                    # Keep resolution information intact
                    filename = filename.replace(f"[{item}]", f" {item} ", 1)
                    metadata["replaced"].append(f"[{item}] -> '{item}' (resolution)")
                    metadata["other_brackets_processed"].append(item)
                    if debug:
                        print(f"  Preserving resolution: '[{item}]'")
                else:
