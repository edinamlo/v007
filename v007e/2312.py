...
def extract_anime_episode(name):
    # Only match 01, 02, Ep02, Ep.20 etc., not plain single digits
    match = re.search(r"(?:\b|_)(?:EP\.?\s?)(\d{2,3})(?:\b|_)", name, re.I)
    if not match:
        match = re.search(r"[\s\-_.](\d{2,3})(?=\s|\[|\(|$)", name)
    if match:
...
def parse_filename(name):
    ...
    left, right = name, ''
    if resolution:
        parts = name.rsplit(resolution, 1)
        if len(parts) == 2:
    ...
    # Title cleaning (basic)
    title = re.sub(r"[._]+", " ", left).strip()

    # --- ANIME GROUP EXTRACTION (LEFT FIRST, THEN RIGHT) ---
    ...
    print(f"Is Anime: {anime_group is not None}, Anime group: {anime_group}, Anime ep: {anime_ep}")
    print(f"Is TV: {tv_match is not None}, TV match: {tv_match}")
    print(f"Is Movie: {year is not None}, Movie Year: {year}")
