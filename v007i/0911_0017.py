...
def extract_tv(name):
    ...
    match = re.search(r"S(\d{1,2})E(\d{1,2})(?:\d{2})?(?=\D|$)", name, re.I)
    if match:
    ...
    if match:
        season_range = f"{match.group(1)}-{match.group(2)}"
        print(f"[TVExtractor] Matched season range {season_range}")
        return (season_range, None), f"found tv season range: s{season_range}", match.group(0)
