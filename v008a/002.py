...
def parse_filename(filename):
    ...
    for key, pattern in CLUE_PATTERNS:
        ...
        if matches:
            ...
            matched_clues.append(matched_text)
    ...
    for clue in matched_clues:
        temp_title = temp_title.replace(clue, '')
    
    # Clean up anime group and other specific patterns
    if result['folder_anime_group']:
        temp_title = temp_title.replace(f"[{result['folder_anime_group']}]", '').strip()
    
    if result['folder_anime_episode']:
        # Handle the anime episode number removal
        temp_title = re.sub(RE_ANIME_EPISODE, '', temp_title)
    
    # Add website cleaner for the left split
    temp_title = RE_WEBSITE_CLEANER.sub('', temp_title)
    
    # Set the title and clean it up
    result['folder_title'] = re.sub(r'[._-]', ' ', temp_title).strip()
    result['folder_title'] = re.sub(r'\s{2,}', ' ', result['folder_title']).strip()
    
    # Determine extra bits by removing everything we found from the original filename
    temp_extras = filename
    for clue in [result['folder_resolution'], result['folder_year'], result['folder_anime_group'], result['folder_anime_episode']]:
        if clue:
            temp_extras = temp_extras.replace(str(clue), '')
    
    # Final cleanup of extras
    result['folder_extra_bits'] = re.sub(r'[._-]', ' ', temp_extras).strip()
    result['folder_extra_bits'] = re.sub(r'\[\s*\]|\(|\)|{.*}', '', result['folder_extra_bits']).strip()
    result['folder_extra_bits'] = re.sub(RE_WEBSITE_CLEANER, '', result['folder_extra_bits'])
