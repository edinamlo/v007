...
def parse_filename(filename):
    ...
    for key, pattern in CLUE_PATTERNS:
        ...
        if match:
            ...
            break # Stop after the first right-to-left match
    
    # If no split was made, use the entire string for leftsplit
    if not matched_clue_text:
        leftsplit = filename
    
    # 2. Iterative leftsplit Extraction and Cleanup
    working_leftsplit = leftsplit
    
    # Website cleaner on leftsplit
    website_match = RE_WEBSITE_CLEANER.search(working_leftsplit)
    if website_match:
        working_leftsplit = working_leftsplit.replace(website_match.group(0), '').strip()
    
    # Remaining clue extraction (year, episodes etc) from leftsplit
    for key, pattern in CLUE_PATTERNS:
        if result.get(f'folder_{key}') is None: # Only search if not already found
            match = pattern.search(working_leftsplit)
            if match:
                ...
                working_leftsplit = working_leftsplit.replace(matched_text, '').strip()

    # 3. Anime Group Parsing (Bidirectional Search)
    anime_group_found = None
    
    # Search working_leftsplit from left-to-right
    bracket_matches_left = RE_BRACKET_GROUP.findall(working_leftsplit)
    ...
    # Clean up the leftsplit to get the title
    final_title = re.sub(r'\[[^\]]+\]|\([^)]+\)|\s{2,}', ' ', working_leftsplit).strip()
    result['folder_title'] = re.sub(r'[._-]', ' ', final_title).strip()
    
    # Combine remaining bits into extra_bits
    all_known_bits = []
    if result['folder_resolution']: all_known_bits.append(result['folder_resolution'])
    if result['folder_year']: all_known_bits.append(result['folder_year'])
    if result['folder_anime_group']: all_known_bits.append(result['folder_anime_group'])
    if result['folder_anime_episode']: all_known_bits.append(result['folder_anime_episode'])
    
    extra_bits_temp = filename
    for bit in all_known_bits:
        extra_bits_temp = extra_bits_temp.replace(str(bit), '').strip()
    
    # Remove the final title to get the leftovers
