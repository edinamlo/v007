...
RE_ANIME_EPISODE_SINGLE = re.compile(r" - (\d{1,3})\b", flags=re.I)
RE_ANIME_EPISODE_DASH = re.compile(r" - (\d{1,3})(?=\s|$)", flags=re.I)
...
CLUE_PATTERNS = [
    ...
    ('year', RE_YEAR),  # Move year before anime_episode to prioritize year detection
    ...
    ('anime_episode_dash', RE_ANIME_EPISODE_DASH),
]
    ...
    while True:
        ...
        for key, pattern in CLUE_PATTERNS:
            ...
            if match:
                ...
                elif key in ['season_ep', 'anime_range', 'anime_episode', 'anime_episode_4digit', 'anime_episode_single', 'anime_episode_dash', 'season_only']:
