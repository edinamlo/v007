...
# New pattern for episode with version (e.g., Ep02v2)
RE_ANIME_EP_VERSION = re.compile(r"\bEp(\d{1,3})(v\d+)\b", flags=re.I)
...
CLUE_PATTERNS = [
    ...
    ('anime_ep_version', RE_ANIME_EP_VERSION),  # Check this before regular episode patterns
    ...
    while True:
        ...
        for key, pattern in CLUE_PATTERNS:
            ...
            if match:
                ...
                elif key == 'anime_ep_version':
                    # Handle episode with version (e.g., Ep02v2)
                    result['folder_media_type'] = 'tv_show'
                    result['folder_is_anime'] = True
                    result['folder_anime_episode'] = match.group(1)
                    # Add version to extra bits
                    result['folder_extra_bits'] += f" {match.group(2)}"
                    # Remove the entire match from working string
                    working_string = working_string[:match.start()].strip()
                    clue_found_in_pass = True
                    if debug_output:
                        logging.info(f"Extractor '{key}' matched '{matched_text}'")
                        logging.info(f"  Episode: '{match.group(1)}', Version: '{match.group(2)}'")
                ...
                if key != 'anime_ep_version':  # Already logged above
                    if debug_output:
                        logging.info(f"Extractor '{key}' matched '{matched_text}'")
                        logging.info(f"  right_text: '{right_text}'")
                        logging.info(f"  left_split: '{working_string}'")
